...
The biography page uses a dedicated Cloudinary image:

```text
bio-lia.jpg
```

The image is delivered dynamically using Cloudinary transformations rather than maintaining separate local image variants.

The current Bio implementation uses:
...
The image is rendered as a semantic `<img>` rather than a CSS background image so that it remains accessible and participates correctly in the document structure.
...
The Bio page is a dedicated editorial biography rather than a generic "About" template.
...
┌─────────────────────┬─────────────────────────────────┐
│                     │                                 │
│     Portrait        │ Lía Lugilde                     │
│                     │ Gijón, 1994                     │
│                     │                                 │
│ Gijón, Asturias     │ Biography                       │
│ Director / Writer   │                                 │
│ Editor / Programmer │                                 │
│                     │                                 │
│ Social              │                                 │
...
On mobile the layout becomes a single editorial column:
...
Filmografía →   Contacto →

Footer
```

The Filmografía and Contacto links are intentionally moved below the complete biography on mobile rather than interrupting the reading flow.
...
The portrait uses a descriptive alternative:
...
A reusable `SEO.tsx` component is planned as part of the global SEO pass.
...
Editorial biography page for Lía Lugilde.

Handles the portrait, biography content, social links and contextual navigation.
...
The project has moved from **functional implementation** into the final production-readiness and refinement phase.

The main content pages are now implemented.
...
The global legal/informational layer is also in place:
...
The Bio page is now implemented as a dedicated editorial biography rather than the original generic template. It uses Lía Lugilde's real biography, a Cloudinary portrait, social links and contextual navigation to the Filmografía and Contacto pages. The navigation adapts its position on mobile so that the biography can be read without interruption.
